...
CBaseNode *CMaxInterface::GetNode(const std::string& strName)
{
  ...
  wchar_t name16[256];
  mbstowcs( name16, strName.c_str(), 256 );

  // create the max node
  if(!pNode->Create(m_pInterface->GetINodeByName(name16)))
  {
    delete pNode;
    return 0;
  }
...
{
  wchar_t buf16[256];
  mbstowcs( buf16, strText.c_str(), 256 );

  m_pInterface->ProgressStart(buf16, true, ProgressFunction, 0);
}
