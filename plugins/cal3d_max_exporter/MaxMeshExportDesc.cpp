...
{	
	const char*	Filefullpathfilename		;
	const char*	Skeletonfullpathfilename	;
	int		MaxNumOfBones				;
  ...
  type_check(arg_list[0], String		, _T("[The first argument of ExportCalMesh should be a string that is a full path name of the file to export]"));
	...
	try
	{
		Filefullpathfilename		= TStr16To8(arg_list[0]->to_string());
		Skeletonfullpathfilename	= TStr16To8(arg_list[1]->to_string());
    ...
    if ( CMaxMeshExport::ExportMeshFromMaxscriptCall(arg_list[0]->to_string(), param) ) {
      // reset to default
