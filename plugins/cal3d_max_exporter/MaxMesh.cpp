...
CMaxMesh::~CMaxMesh()
{
	// Generates a crash in max 2010 x64. Don't know exactly why.
...
bool CMaxMesh::AddBoneInfluence(CSkeletonCandidate *pSkeletonCandidate, CVertexCandidate *pVertexCandidate, INode *pNode, float weight)
{
  ...
  int boneId = pSkeletonCandidate->GetBoneId( TStr16To8(pNode->GetName()));
...
bool CMaxMesh::CreateMaterial(Mtl *pMtl)
{
  ...
  if(pMtl->ClassID() == Class_ID(DMTL_CLASS_ID, 0))
  {
    ...
    SInts face_ids;
    for( int i=0; i<m_pIMesh->getNumFaces(); ++i ) {
        int  materialId = m_pIMesh->getFaceMtlIndex( i );
        face_ids.insert( materialId );
    }
 
    for( size_t i=1; i<face_ids.size(); ++i )
        m_vectorStdMat.push_back((StdMat *)pMtl);
...
//----------------------------------------------------------------------------//
// Added by JLA / Novarama
int CMaxMesh::GetFaceMaterialMaxID(int faceId) {
...
int CMaxMesh::GetSubmeshMaterialThreadId(int submeshId)
{
  ...
  std::string strName = TStr16To8(pStdMat->GetName()).s8;
...
CVertexCandidate *CMaxMesh::GetVertexCandidate(CSkeletonCandidate *pSkeletonCandidate, int faceId, int faceVertexId)
{
  ...
  if(m_modifierType == MODIFIER_PHYSIQUE)
  {
    ...
    else if(vertexType == RIGID_BLENDED_TYPE)
    {
      ...
      {
        // add the influence to the vertex candidate
				INode *n = pTypeVertex->GetNode(nodeId);
        if(!AddBoneInfluence(pSkeletonCandidate, pVertexCandidate, n, pTypeVertex->GetWeight(nodeId)))
        {
					char msg[ 512 ];
					sprintf( msg, "The mesh is binded to bone %s which is not in the skeleton definition!!", n->GetName() );
					MessageBox( NULL, TStr8To16(msg), __T("Export error"), MB_OK );
					pPhysiqueExport->ReleaseContextInterface(pContextExport);
          m_pModifier->ReleaseInterface(I_PHYINTERFACE, pPhysiqueExport);
          delete pVertexCandidate;
          theExporter.SetLastError("Invalid bone assignment.", __FILE__, __LINE__);
...
Point3 CMaxMesh::GetVertexNormal(int faceId, int vertexId)
{
  ...
  // NOVARAMA added for check not created Rvertexs
  // Max 2012 seems to don't check if rvert member pointer is NULL
  // This call regenerates
  if( m_pIMesh->getRVertPtr( 0 ) == NULL ) {
    checkHostData( );
  }

  // get the "rendered" vertex
  RVertex *pRVertex = m_pIMesh->getRVertPtr(vertexId);
