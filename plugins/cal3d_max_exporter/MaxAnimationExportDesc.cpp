...
{	
	int i								;
	const wchar_t*	Filefullpathfilename		;
	const wchar_t*	Skeletonfullpathfilename	;
	Array*	BonesArray					;
	int		StartFrame					;
	int		EndFrame					;
	int		FrameOffset					;
  ...
  // Cedric Pinson, now we can export in gl coordinates
	...
	type_check(transform, Boolean, _T("[The axisGL argument of ExportCalAnim should be a boolean that is true if you want to export in openGL axis]"));
		...
		if (! _tcscmp(Filefullpathfilename,_T("")))return new Integer(1);

		if (! _tcscmp(Skeletonfullpathfilename,_T(""))) return new Integer(2);


   char name8[ 256 ];
   wcstombs( name8, Skeletonfullpathfilename, 256 );

		//Does skeleton file exist ?
		FILE* _stream;
		_stream = fopen(name8,"r");
    ...
    bUseAxisGL       = (transform->to_bool() != 0);
		...
		AnimExportParams param(name8, tabnode, StartFrame, EndFrame, FrameOffset, FrameRate);
