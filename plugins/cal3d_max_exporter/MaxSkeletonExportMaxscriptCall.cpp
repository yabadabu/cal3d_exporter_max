...
bool CSkeletonCandidate::CreateFromInterfaceFromMaxScriptCall()
{
	...
	for( size_t i=0; i<m_vectorBoneCandidate.size(); ++i ) 
		m_vectorBoneCandidate[ i ]->SetSelected( false );

	// A second pass to just select those bones selected from ms
	for(nodeId = 0; nodeId < NumNodes; nodeId++) {
		CBaseNode* basenode = imax->GetNodeFromMaxscriptArray(nodeId);
		CMaxNode* ms_max_node = (CMaxNode*) basenode;
		for( size_t i=0; i<m_vectorBoneCandidate.size(); ++i ) {
			CMaxNode *max_node = ( CMaxNode * ) m_vectorBoneCandidate[ i ]->GetNode( );
			if( max_node->GetINode( ) == ms_max_node->GetINode( ) ) 
				m_vectorBoneCandidate[ i ]->SetSelected( true );
		}
	}	
	// Added by JLA ----------------------------
