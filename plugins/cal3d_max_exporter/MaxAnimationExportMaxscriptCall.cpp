...
bool CMaxInterface::ExportAnimationFromMaxscriptCall(const std::string& strFilename, void* _AnimExportParams)
{
	...
	*/
	...
	//Following block replaces the user interface interactions
	{
		...
		{
			AfxMessageBox(theExporter.GetLastErrorW(), MB_OK | MB_ICONEXCLAMATION);
			return false;
		}	
        ...
        char bonename8[ 256 ];
        wcstombs( bonename8, param->m_tabbones[j]->GetName(), 256 );
				std::string	bonename	= bonename8;

				if (bcname == bonename)
				{
          INode *max_real_inode = param->m_tabbones[j];

          // ---------------------- Added by JLA
          // Remove default candidate node found using GetINodeByName 
          CBaseNode *old_node = bonecandidate->GetNode( );
          if( old_node )
            delete old_node;

          // Create a new max node to hold the real INode from max
          CMaxNode *max_node = new CMaxNode;
          max_node->Create( max_real_inode );

          // Update the bone from the skeleton candidate to use the real bone
          bonecandidate->SetNode( max_node );
          // ---------------------- Added by JLA
