...
{	
	...
	const wchar_t*	fullpathfilename16;
	int			ArraySize		;
	bool		bShowUI			;
  ...
  // Cedric Pinson, now we can export in gl coordinates
	...
	try
	{
    fullpathfilename	= TStr16To8( arg_list[0]->to_string() );
		fullpathfilename16	= arg_list[0]->to_string();
    ...
    if (CMaxSkeletonExport::ExportSkeletonFromMaxscriptCall(fullpathfilename16,tabnode, bShowUI) ) {
      // reset axis gl
