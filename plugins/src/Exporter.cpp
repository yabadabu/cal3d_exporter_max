...
bool CExporter::ExportAnimation(const std::string& strFilename)
{
  ...
  CAnimationExportSheet sheet(L"Cal3D Animation Export", m_pInterface->GetMainWnd());
  ...
  if(selectedCount == 0)
  {
    SetLastError(std::string( "No bones selected to export."), __FILE__, __LINE__);
    return false;
  }
  ...
  int endFrame = roundToInt(duration * (float)sheet.GetFps());
  int displacedFrame = roundToInt(((float)sheet.GetDisplacement() / (float)m_pInterface->GetFps()) * (float)sheet.GetFps()) % endFrame;
...
bool CExporter::ExportMesh(const std::string& strFilename)
{
  ...
  // show export wizard sheet
  CMeshExportSheet sheet(L"Cal3D Mesh Export", m_pInterface->GetMainWnd());
  sheet.SetSkeletonCandidate(&skeletonCandidate);
  ...
  // start the progress info
  m_pInterface->StartProgressInfo(std::string( "Exporting to mesh file..."));
...
bool CExporter::ExportSkeleton(const std::string& strFilename)
{
  ...
  HKEY hk;
  LONG lret=RegCreateKey(HKEY_CURRENT_USER, L"Software\\Cal3D\\Exporter", &hk);
  if(lret==ERROR_SUCCESS && NULL!=hk)
  {
    lret=RegSetValueEx(hk,L"skeleton",NULL,REG_SZ,(unsigned char *)strFilename.c_str() ,strFilename.length());
    RegCloseKey(hk);
  }
...
const wchar_t *CExporter::GetLastErrorW() {
  static wchar_t buf16[256];
  mbstowcs( buf16, m_strLastError.c_str(), 256 );
  return buf16;
}
