...
BOOL CMaterialMapsPage::BeginPage()
{
    ...
    wchar_t buf[ 256 ];
    mbtowc( buf, vectorMap[mapId].strFilename.c_str(), vectorMap[mapId].strFilename.length() );
		m_mapsCtrl.SetItemText(hItem, 1, buf );
	}
...
void CMaterialMapsPage::OnMapDoubleClick(NMHDR* pNMHDR, LRESULT* pResult) 
{
  ...
  if(selectedItem >= 0)
  {
    ...
    if(dlg.DoModal() == IDOK)
    {
      ...
      char buf8[ 256 ];
      wcstombs( buf8, dlg.GetFilename(), 256 );
      vectorMap[selectedItem].strFilename = buf8;
