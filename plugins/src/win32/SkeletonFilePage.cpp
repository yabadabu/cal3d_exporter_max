...
BOOL CSkeletonFilePage::BeginPage()
{
	HKEY hk; 
	DWORD dwtype;
	LONG lret=RegOpenKeyEx(HKEY_CURRENT_USER,_T("Software\\Cal3D\\Exporter"),NULL,KEY_READ|KEY_WRITE|KEY_SET_VALUE,&hk);
	if(lret==ERROR_SUCCESS && NULL!=hk)
	{
		unsigned char pbuf[256];
        DWORD dwlen=sizeof(pbuf);

		const TCHAR* valname=_T("skeleton");
        lret=RegQueryValueEx(hk,valname,NULL,&dwtype,pbuf,&dwlen);
		if(lret==ERROR_SUCCESS)
		{
			m_lruCombo.SetWindowText((TCHAR*)pbuf);			
		}		
		RegCloseKey(hk);
	}
...
LRESULT CSkeletonFilePage::EndPage()
{
  ...
  /*
  si
ze_t origsize = strlen(orig) + 1;
  size_t convertedChars = 0;
  wchar_t wcstring[newsize];
  mbstowcs_s(&convertedChars, wcstring, strFilename.GetString(), (char *) strFilename, _TRUNCATE);
  */
	// create the skeleton candidate from the skeleton file
  const wchar_t *buf16 = strFilename.GetString();
  char buf8[ 64 ];
  wcstombs( buf8, buf16, 64 );
	if(!m_pSkeletonCandidate->CreateFromSkeletonFile( buf8 ) )
	{
		//AfxMessageBox(theExporter.GetLastError().c_str(), MB_OK | MB_ICONEXCLAMATION);
		return -1;
	}
...
void CSkeletonFilePage::OnBrowse() 
{
	...
	if(lret==ERROR_SUCCESS && NULL!=hk)
	{
		lret=RegSetValueEx(hk,_T("skeleton"),NULL,REG_SZ,(unsigned char *)strFilename.GetBuffer(1) ,strFilename.GetLength());
		RegCloseKey(hk);
	}
